...
class AuthController extends CI_Controller {
	...
	public function login(){
		...
		if($result){
			...
			$query = $this->db->query("SELECT * from tbl_account WHERE EMAIL_ADDRESS = '$email'; ");
			...
			foreach ($query->result() as $row)
			{
					...
					$ACCOUNT_TYPE = $row->ACCOUNT_TYPE_NUMBER;
		...
		if ($ACCOUNT_TYPE== 'ACC_TYPE_NGSI-100449')
		{
			redirect(base_url('AuthController/dash'));
		}
		elseif($ACCOUNT_TYPE== 'ACC_TYPE_NGSI-100450')
		{
			redirect(base_url('AuthController/dash_bidder'));
		}
		else
		{
		...
		echo '<script type="text/javascript">
		   window.location = "http://localhost/ci-bidding/"
		  </script>';
	...
	public function dash_bidder(){
		if($this->session->userdata('user')){
			
			$this->load->view('bidder/dashboard_bidder');
		}
		else{
			redirect('/');
		}
	...
	public function reg_profile(){
		...
		if($this->input->post()){
			...
			if($userId){
				$this->session->set_flashdata('success','User Registered Successfully.');
			echo '<script type="text/javascript">
           	window.location = "http://localhost/ci-bidding/AuthController/dash"
      		</script>';
			}
			else
			{
	...
	public function verify_user(){
		...
		if($this->input->post()){
			...
			$member_number = "MBR-NUM-100448";
			$contact_numeber = $this->input->post('contact_numeber');
			$first_name = $this->input->post('first_name');
			$last_name = $this->input->post('last_name');
			$gender = $this->input->post('gender');
			$birthday = $this->input->post('birthday');
